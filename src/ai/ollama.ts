...
`;
  ...
  try {
    const response = await fetch(`${aiApiUrl}/api/chat`, {
      ...
      body: JSON.stringify({
        model: "domi-coder-select", // 커스텀 모델명으로 변경
        messages: [
          { role: "user", content: userPrompt }, // 시스템 프롬프트 미지입 (모델 자체에 빌트인됨)
        ],
        format: "json",
...
`;
  ...
  try {
    const response = await fetch(`${aiApiUrl}/api/chat`, {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        model: "domi-coder-update", // 커스텀 모델명으로 변경
        messages: [
          { role: "user", content: userPrompt }, // 시스템 프롬프트 미지입 (모델 자체에 빌트인됨)
        ],
        format: "json",
    ...
    if (json.message && json.message.content) {
      try {
        ...
        console.error("❌ Ollama 파싱 실패 원본 내용:", json.message.content);
