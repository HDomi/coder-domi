...
export async function generateCodeUpdate(
  ...
  console.log(
    `[Prompt Diet] 1단계: 의존 파일 선별 분석 시작... (전체 파일 수: ${workspaceContext.files.length}개)`,
  );
  ...
  console.log(
    `[Prompt Diet] 2단계: 핵심 소스 코드 전송 시작... (다이어트 후 파일 수: ${prunedFiles.length}개 / ${allPaths.length}개)`,
  );
  console.log(
